		...
		while(programRunning) {
			...
			try {
				...
				case 7:
					...
					System.out.println("Added " + quantAdd + " " + plural + " of " + cards.get(cardAdd) + " to " + decks.get(deckAdd).getDeckName() + ".");
				...
				case 8:
					...
					if(decks.get(deckRem).existsInDeck(cards.get(cardRem))) {
						...
						if(quantRem > realCount) quantRem = realCount;
						
						decks.get(deckRem).removeCard(cards.get(cardRem), quantRem);
						
						String plural2 = quantRem == 1 ? "copy" : "copies";
						
						System.out.println("Removed " + quantRem + " " + plural2 + " of " + cards.get(cardRem) + " from " + decks.get(deckRem).getDeckName() + ".");
